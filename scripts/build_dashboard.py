...
def card(a: dict, *, is_preferito: bool = False, in_preferiti_set: set | None = None) -> str:
    ...
    # Heart button (cliccabile via JS, copia comando per chat)
    already_fav = (in_preferiti_set and a.get("id") in in_preferiti_set) or is_preferito
    if is_preferito:
        heart_html = '<div class="heart-badge filled" title="Nei preferiti">❤️</div>'
    elif already_fav:
        heart_html = '<div class="heart-badge filled" title="Anche nei preferiti">❤️</div>'
    else:
        heart_html = (
            f'<button class="heart-btn" onclick="favorite(\'{url}\', \'{a.get("titolo","").replace(chr(39), chr(92)+chr(39))[:60]}\')" '
...
<head>
  ...
  <style>
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .tip {{ background: #fef3c7; border-left: 4px solid #f59e0b; padding: 12px 16px; margin: 12px 20px; border-radius: 6px; font-size: 13px; color: #78350f; max-width: 900px; margin-left: auto; margin-right: auto; }}
    ...
    .heart-btn, .heart-badge {{ background: none; border: none; font-size: 24px; cursor: pointer; padding: 4px; border-radius: 50%; transition: transform .15s; line-height: 1; }}
    .heart-btn:hover {{ transform: scale(1.15); background: #fce7f3; }}
...
<body>
  ...
  <div class="tip">
    💡 <strong>Per salvare un annuncio nei preferiti</strong>: clicca il 🤍 nell'angolo della card. Il comando viene copiato negli appunti — incollalo nella chat con Claude per salvarlo definitivamente.
  </div>
  <div class="main">
    {preferiti_section}
  ...
  <script>
  function favorite(url, titolo) {{
    const cmd = 'preferito: ' + url + '\\nnote: (aggiungi una nota qui se vuoi)';
    navigator.clipboard.writeText(cmd).then(() => {{
      showToast('✓ Comando copiato! Incollalo nella chat con Claude per salvare "' + titolo + '" nei preferiti.');
    }}).catch(() => {{
      prompt('Copia questo testo e incollalo nella chat con Claude:', cmd);
    }});
  }}
